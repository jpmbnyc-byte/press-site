...
export default function NewsletterSignup({ source = 'homepage' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err?.message ||
          'Could not save your email just now. Please try again in a moment.'
      );
    } finally {
      setLoading(false);
    }
  ...
  if (submitted) {
    return (
      <div className="max-w-md mx-auto text-center">
        <div className="font-mono text-[9px] tracking-[0.3em] uppercase text-[var(--hw-gold)] mb-5">
          The Weekly Dispatch
        </div>
        <p className="font-serif text-3xl italic font-light text-[var(--hw-ink)] leading-snug">
          You are on the list.<br />Peace be with you.
        </p>
        <p className="font-mono text-[8px] tracking-[0.15em] uppercase text-[var(--hw-ink3)] mt-8">
          We&apos;ll write when the next dispatch goes out · Unsubscribe any time
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      ...
      <p className="font-serif italic text-lg text-[var(--hw-ink2)] mb-8">
        Leave your email for editorial notes from the field. No noise. No membership required.
      </p>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="text-left">
          <label htmlFor={`dispatch-name-${source}`} className="sr-only">
            Name
          </label>
          <input
            id={`dispatch-name-${source}`}
            type="text"
            name="name"
            autoComplete="name"
            placeholder="Name"
        ...
        <div className="text-left">
          <label htmlFor={`dispatch-email-${source}`} className="sr-only">
            Email
          </label>
          <input
        ...
        {error && (
          <p className="font-serif text-sm text-[var(--hw-rust)]" role="alert">
            {error}
          </p>
        )}
        <button
        ...
        >
          {loading ? 'Saving…' : 'Join the list →'}
        </button>
      </form>
      <p className="font-mono text-[8px] tracking-[0.15em] uppercase text-[var(--hw-ink3)] mt-6">
        Free email list · Sent when there is something worth saying
      </p>
    </div>
  );
}
